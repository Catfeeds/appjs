...
class GoodsController extends Controller
{
    ...
    public function actionGoodsUpDown($id = 0, $type = 'down')
    {
        if ($type == 'down') {
            ...
            if (!$goods->getNum()) {
                $return_url = \Yii::$app->urlManager->createUrl(['admin/goods/goods-attr', 'id' => $goods->id]);
                if (!$goods->use_attr)
                    $return_url = \Yii::$app->urlManager->createUrl(['admin/goods/goods-edit', 'id' => $goods->id]) . '#step3';
                $this->renderJson([
                    'code' => 1,
                    'msg' => '商品库存不足，请先完善商品库存',
