...
use app\modules\admin\models\group\PtCatForm;
use app\modules\admin\models\group\PtGoodsForm;
        ...
        if ($type == 'down') {
            ...
            if (!PtGoods::getNum($goods->id)) {
                $this->renderJson([
                    'code' => 1,
                    'msg' => '商品库存不足，请先完善商品库存',
                    'return_url' => \Yii::$app->urlManager->createUrl(['admin/group/goods/goods-attr', 'id' => $goods->id]),
                ]);
            }
            $goods->status = 1;
