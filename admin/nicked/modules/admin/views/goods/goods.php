    ...
    <div class="panel-body">
        ...
        <div class="mb-3 clearfix">
            <div class="float-left">
                <a href="<?= $urlManager->createUrl(['admin/goods/goods-edit']) ?>" class="btn btn-primary"><i
                            class="iconfont icon-playlistadd"></i>添加商品</a>
                <a href="javascript:void(0)" class="btn btn-secondary batch"
                   data-url="<?= $urlManager->createUrl(['admin/goods/batch']) ?>" data-content="是否批量上架"
                   data-type="0">批量上架</a>
                <a href="javascript:void(0)" class="btn btn-warning batch"
                   data-url="<?= $urlManager->createUrl(['admin/goods/batch']) ?>" data-content="是否批量下架"
                   data-type="1">批量下架</a>
                <a href="javascript:void(0)" class="btn btn-danger batch"
                   data-url="<?= $urlManager->createUrl(['admin/goods/batch']) ?>" data-content="是否批量删除"
                   data-type="2">批量删除</a>
                ...
                <div class="dropdown float-right ml-2">
                    ...
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton"
                        ...
                        <a class="dropdown-item" href="<?= $urlManager->createUrl(['admin/goods/goods']) ?>">全部类型</a>
                        <?php foreach ($cat_list as $index => $value): ?>
                            <a class="dropdown-item"
                               href="<?= $urlManager->createUrl(array_merge(['admin/goods/goods'], $_GET, ['cat' => $value])) ?>"><?= $value ?></a>
                        <?php endforeach; ?>
                    </div>
                </div>
            </div>
                ...
                <tr>
                    ...
                    <td class="nowrap">
                        <?php if ($goods->use_attr): ?>
                            <a href="<?= $urlManager->createUrl(['admin/goods/goods-attr', 'id' => $goods->id]) ?>"><?= $goods->num ?></a>
                        <?php else: ?>
                            <a href="<?= $urlManager->createUrl(['admin/goods/goods-edit', 'id' => $goods->id]) ?>#step3"><?= $goods->num ?></a>
                        <?php endif; ?>
                    </td>
                    <td class="nowrap">
                    ...
                    <td class="nowrap">
                        <a class="btn btn-sm btn-primary"
                           href="<?= $urlManager->createUrl(['admin/goods/goods-edit', 'id' => $goods->id]) ?>">修改</a>
                        ...
                        <a class="btn btn-sm btn-danger del"
                           href="<?= $urlManager->createUrl(['admin/goods/goods-del', 'id' => $goods->id]) ?>">删除</a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
...
<script>
    ...
    function upDown(id, type) {
        ...
        var url = "<?= $urlManager->createUrl(['admin/goods/goods-up-down']) ?>";
        if (confirm("是否" + text + "？")) {
            $.ajax({
                url: url,
                type: 'get',
...
//        var more = $('input[name^="integral[more]"]').val();
        ...
        $.ajax({
            url: "<?= Yii::$app->urlManager->createUrl(['admin/goods/batch-integral']) ?>",
            type: 'get',
            dataType: 'json',
            data: {
