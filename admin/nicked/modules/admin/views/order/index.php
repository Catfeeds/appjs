                    ...
                    <tr>
                        ...
                        <td class="order-tab-5">
                                ...
                                <?php if ($order_item['is_delete'] == 0): ?>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-primary apply-status-btn"
                                           href="<?= $urlManager->createUrl(['admin/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 1]) ?>">同意请求</a>
                                    </div>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-danger apply-status-btn"
                                           href="<?= $urlManager->createUrl(['admin/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 0]) ?>">拒绝请求</a>
                                    </div>
                                <?php endif; ?>
                            <?php else: ?>
                            ...
                            <a class="btn btn-sm btn-primary"
                               href="<?= $urlManager->createUrl(['admin/order/detail', 'order_id' => $order_item['id']]) ?>">详情</a>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="5">
...
<script>
    ...
    $(document).on("click", ".send-confirm-btn", function () {
        ...
        console.log(error);
        $.ajax({
            url: "<?=$urlManager->createUrl(['admin/order/send'])?>",
            type: "post",
...
<script>
    ...
    $(document).on('click', '.print', function () {
        ...
        $.ajax({
            url: "<?=$urlManager->createAbsoluteUrl(['admin/order/print'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                id: id,
...
<script>
    ...
    $(document).on('click', '.add-price', function () {
        ...
        error.hide();
        $.ajax({
            url: "<?=$urlManager->createUrl(['admin/order/add-price'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                order_id: order_id,
