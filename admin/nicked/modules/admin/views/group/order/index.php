...
<script language="JavaScript" src="<?= $statics ?>/admin/js/LodopFuncs.js"></script>
            ...
            <ul class="nav nav-tabs status">
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == -1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/index'])) ?>">全部</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 0 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/index'], $condition, ['status' => 0])) ?>">未付款</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/index'], $condition, ['status' => 1])) ?>">待发货</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 2 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/index'], $condition, ['status' => 2])) ?>">已发货</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 6 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/index'], $condition, ['status' => 6])) ?>">拼团中</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 3 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/index'], $condition, ['status' => 3])) ?>">已完成</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 5 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/index'], $condition, ['status' => 5])) ?>">已取消</a>
                </li>
            </ul>
        </div>
...
<script>
    ...
    $(document).on("click", ".send-confirm-btn", function () {
        ...
        console.log(error);
        $.ajax({
            url: "<?=$urlManager->createUrl(['admin/group/order/send'])?>",
            type: "post",
...
<script>
    ...
    $(document).on('click', '.print', function () {
        ...
        $.ajax({
            url: "<?=$urlManager->createAbsoluteUrl(['admin/group/order/print'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                id: id,
