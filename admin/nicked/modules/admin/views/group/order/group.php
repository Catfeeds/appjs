            ...
            <ul class="nav nav-tabs status">
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == -1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/group'])) ?>">全部</a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 0 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/group'], $condition, ['status' => 0])) ?>">拼团中<?= isset($store_data['status_count']['status_0']) ? '(' . $store_data['status_count']['status_0'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 1 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/group'], $condition, ['status' => 1])) ?>">拼团成功<?= isset($store_data['status_count']['status_1']) ? '(' . $store_data['status_count']['status_1'] . ')' : null ?></a>
                </li>
                <li class="nav-item">
                    <a class="status-item nav-link <?= $status == 2 ? 'active' : null ?>"
                       href="<?= $urlManager->createUrl(array_merge(['admin/group/order/group'], $condition, ['status' => 2])) ?>">拼团失败<?= isset($store_data['status_count']['status_2']) ? '(' . $store_data['status_count']['status_2'] . ')' : null ?></a>
                </li>
            </ul>
        </div>
                    ...
                    <tr>
                        ...
                        <td class="order-tab-5">
                            <a class="btn btn-sm btn-primary"
                               href="<?= $urlManager->createUrl(['admin/group/order/group-list', 'pid' => $order_item['id']]); ?>">团信息</a>
                        </td>
                    </tr>
                </table>
            </div>
