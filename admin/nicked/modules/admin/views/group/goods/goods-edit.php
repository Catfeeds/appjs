...
if (!$returnUrl)
    $returnUrl = $urlManager->createUrl(['admin/group/goods/index']);
?>
<script src="<?= $staticBaseUrl ?>/admin/js/uploadVideo.js"></script>
<style>
    .cat-box {
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                <div>请前往商品库存设置库存和价格，<a target="_blank"
                                                       href="<?= $urlManager->createUrl(['admin/group/goods/goods-attr', 'id' => $goods['id']]) ?>">立即前往</a>
                                </div>
                            </div>
                        </div>
                    <?php else: ?>
...
<script>
    ...
    function getSubCatList() {
        ...
        page.sub_cat_list = [];
        $.ajax({
            url: "<?=$urlManager->createUrl(['admin/goods/get-cat-list'])?>",
            data: {
                parent_id: parent_id,
            },
...
<script>
    $(document).on('click', '.copy-btn', function () {
        ...
        $.myLoading();
        $.ajax({
            url: "<?=$urlManager->createUrl(['admin/goods/copy'])?>",
            type: 'get',
            dataType: 'json',
            data: {  url: url },
            success: function (res) {
    ...
    $(document).on('click', '.mall-copy-btn', function () {
        ...
        $.myLoading();
        $.ajax({
            url: "<?=$urlManager->createUrl(['admin/group/goods/copy'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                mall_id: mall_id,
