...
<div class="main-nav" flex="cross:center dir:left box:first">
    <div>
        <nav class="breadcrumb rounded-0 mb-0" flex="cross:center">
            <a class="breadcrumb-item" href="<?= $urlManager->createUrl(['admin/store/index']) ?>">我的商城</a>
...
<script>
    $(document).on("click", ".save-attr-btn", function () {
        ...
        error.hide();
        $.ajax({
            url: "<?=$urlManager->createUrl(['admin/store/attr-add'])?>",
            type: "post",
            data: {
                _csrf: _csrf,
    ...
    $(document).on("click", ".attr-delete", function () {
        ...
        btn.btnLoading("正在删除");
        $.ajax({
            url: "<?=$urlManager->createUrl(['admin/store/attr-delete'])?>",
            data: {
                attr_id: attr_id,
            },
            dataType: "json",
    ...
    $(document).on("blur", "tr .attr-edit", function () {
        ...
        if (oldval != newval) {
            $.ajax({
                url: "<?=$urlManager->createUrl(['admin/store/attr-update'])?>",
                type: "post",
                dataType: "json",
                data: {
                    _csrf: _csrf,
