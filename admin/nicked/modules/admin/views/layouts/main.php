...
<head>
    ...
    <link href="<?= Yii::$app->request->baseUrl ?>/statics/admin/css/jquery.datetimepicker.min.css" rel="stylesheet">
    ...
    <link href="<?= Yii::$app->request->baseUrl ?>/statics/admin/css/common.v2.css?version=<?= $version ?>"
          rel="stylesheet">

    <script>var _csrf = "<?=Yii::$app->request->csrfToken?>";</script>
    <script>var _upload_url = "<?=Yii::$app->urlManager->createUrl(['upload/file'])?>";</script>
    <script>var _upload_file_list_url = "<?=Yii::$app->urlManager->createUrl(['admin/store/upload-file-list'])?>";</script>
    ...
    <script src="<?= Yii::$app->request->baseUrl ?>/statics/admin/js/jquery.datetimepicker.full.min.js"></script>
    <script src="<?= Yii::$app->request->baseUrl ?>/statics/js/common.js?version=<?= $version ?>"></script>
    <script src="<?= Yii::$app->request->baseUrl ?>/statics/admin/js/common.v2.js?version=<?= $version ?>"></script>
    <script src="<?= Yii::$app->request->baseUrl ?>/statics/admin/js/clipboard.js"></script>
    <script src="<?= Yii::$app->request->baseUrl ?>/statics/admin/vendor/layer/layer.js"></script>
    <script src="<?= Yii::$app->request->baseUrl ?>/statics/admin/vendor/laydate/laydate.js"></script>
</head>
<body>
...
<div class="sidebar <?= $current_menu && count($current_menu['list']) ? 'sidebar-sub' : null ?>">
    <div class="sidebar-1">
        <div class="logo">
            <a class="home-link"
               href="<?= $urlManager->createUrl(['admin/default/index']) ?>"><?= $this->context->store->name ?></a>
        </div>
        <div>
        ...
        <script>
                    ...
                    success: function (res) {
                        if (res.code == 0) {
                            ...
                            for (var i = 0; i < count; i++) {
                                ...
                                var html = "<a class='dropdown-item' data-index='" + res.data[i].id + "' href='<?=Yii::$app->urlManager->createUrl(['admin/order/index', 'status' => 1])?>'>" + res.data[i].name + "下了一个订单</a>";
            ...
            $(document).ready(function () {
                ...
                $('.message-list').on('click', 'a', function () {
                    ...
                    $.ajax({
                        url: '<?=Yii::$app->urlManager->createUrl(['admin/get-data/message-del'])?>',
                        type: 'get',
                        dataType: 'json',
                        data: {
                            'id': $(this).data('index')
