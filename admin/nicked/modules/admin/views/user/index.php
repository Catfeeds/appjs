    ...
    <div class="panel-body">
        <div class="dropdown float-left">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton"
                ...
                <a class="dropdown-item" href="<?= $urlManager->createUrl(['admin/user/index']) ?>">全部会员</a>
                <?php foreach ($level_list as $index => $value): ?>
                    <a class="dropdown-item"
                       href="<?= $urlManager->createUrl(array_merge(['admin/user/index'], $_GET, ['level' => $value['level'], 'page' => 1])) ?>"><?= $value['name'] ?></a>
                <?php endforeach; ?>
            </div>
        </div>
            ...
            <?php foreach ($list as $u): ?>
                <tr>
                    ...
                    <td>
                        <a class="btn btn-sm btn-link"
                           href="<?= $urlManager->createUrl(['admin/order/index', 'user_id' => $u['id']]) ?>"><?= User::getCount($u['id']) ?></a>
                    </td>
                    <td>
                        <a class="btn btn-sm btn-link"
                           href="<?= $urlManager->createUrl(['admin/user/coupon', 'user_id' => $u['id']]) ?>"><?= User::getCouponcount($u['id']) ?></a>
                    </td>
                    <td>
                        <a class="btn btn-sm btn-link"
                           href="<?= $urlManager->createUrl(['admin/user/card', 'user_id' => $u['id']]) ?>"><?= User::getCardCount($u['id']) ?></a>
                    </td>
                    <td>
                        <a class="btn btn-sm btn-link"
                           href="<?= $urlManager->createUrl(['admin/user/rechange-log', 'user_id' => $u['id']]) ?>"><?= $u['integral'] ?></a>
                    </td>
                    <td>
                        <a class="btn btn-sm btn-primary"
                           href="<?= $urlManager->createUrl(['admin/user/edit', 'id' => $u['id']]) ?>">编辑</a>
                ...
                <td>
                    <a class="btn btn-sm btn-danger del" href="javascript:"
                       data-url="<?= $urlManager->createUrl(['admin/user/del', 'id' => $u['id']]) ?>"
...
<script>
    ...
    $(document).on('click', '.save-rechange', function () {
        ...
        $.ajax({
            url: "<?= Yii::$app->urlManager->createUrl(['admin/user/rechange']) ?>",
            type: 'post',
            dataType: 'json',
