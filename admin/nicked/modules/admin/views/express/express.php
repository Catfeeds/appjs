...
<div class="p-3">
    <a href="<?= $urlManager->createUrl(['admin/express/express']) ?>">收费地区管理</a>
    <hr>
    <div class="p-3">
        ...
        <hr>
        <a href="<?= $urlManager->createAbsoluteUrl(['admin/express/express-add']) ?>" class="btn btn-primary">添加收费地区</a>
    </div>
    <div class="card">
            ...
            <tbody>
                <?php foreach ($list as $index => $value): ?>
                    <tr>
                        ...
                        <td><?= $value['postage'] ?></td>
                        <td>
                            <a data-toggle="modal" data-target="#replyModal-<?=$value['id']?>" href="<?= $urlManager->createAbsoluteUrl(['admin/good/good-class-edit', 'id' => $value['id']]) ?>">[设置邮费]</a>
                            <a class="del" href="<?= $urlManager->createAbsoluteUrl(['admin/express/express-del', 'id' => $value['id']]) ?>">[取消收费]</a>
...
<script>
    ...
    $(document).on('click','.reply-btn',function(){
        ...
        $.ajax({
            method:'post',
            dataType:'json',
            url:'<?= $urlManager->createAbsoluteUrl(['admin/express/express-add']) ?>',
            data:form.serialize()+'&_csrf='+_csrf,
            success:function(res){
                if (res.code==0) {
                    window.location.href="<?= $urlManager->createAbsoluteUrl(['admin/express/express']) ?>";
                }else{
